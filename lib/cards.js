...
function generateCardHtml(card) {
	...
	if (card.timePeriodTextAlt) {
	cardHtml += '<span class="tool" data-tip="' + card.timePeriodTextAlt + '">'
	}
	
	cardHtml += '<p class="label ' + card.timePeriodClass + '">' + card.timePeriodText + '</p>'
    ...
    if (card.links && card.links.length > 0) {
        ...
        for (var linkIndex = 0; linkIndex < card.links.length; linkIndex++) {
			...
			var linkUrl = (link.url.includes('?') 
				? link.url + '&'
				: link.url + '?') + 'ref=kurtlourens.com';
				
            linksHtml += '<a href="' + linkUrl + '" target="_blank" rel="noopener" title="' + link.alt + '" alt="' + link.alt + '">' + link.text + '</a>';
        }
        linksHtml += '</small>';
        cardHtml += linksHtml;
    } else {
